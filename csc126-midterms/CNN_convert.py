...
folder_path = 'singular_dataset' #########folder path trajectory adjustment here ################
...
if not os.path.exists('debug/CNN_debug_images'): ##debug rani
    print(f"Folder 'CNN_debug_images' does not exist. Creating....")
    os.makedirs('debug/CNN_debug_images')
...
for file in os.listdir(folder_path):
    ...
    arr = np.array(img)
    arr = np.where(arr > 128, 255.0, 0.0) # can adjust the binary threshold for testing langs
    
    CNN_debug_path = os.path.join("debug/CNN_debug_images", file)
